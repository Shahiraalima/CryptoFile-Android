...
public class DecryptFileActivity extends AppCompatActivity {
    ...
    private void displaySelectedFiles() {
        ...
        decryptButton.setEnabled(!selectedFileUris.isEmpty());
        adjustListViewHeight();
    }
    ...
    private void performDecryption(String password) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                ...
                for (int i = 0; i < selectedFileUris.size(); i++) {
                    if (cancelRequested) {
                        final int idx = i;
                        final String cancelledFileName = getFileName(selectedFileUris.get(idx));
                    ...
                    String encryptedHash = calculateFileHash(tempInputFile);

                    boolean isValid = false;
                    try {
                        isValid = fileDAO.validityEncryption(encryptedHash, userId).get();
                    ...
                    if (!isValid) {
                        final int idx = i;
                        runOnUiThread(() -> {
                            displayItems.set(idx, fileName + " - ❌ Invalid encrypted file");
                            filesAdapter.notifyDataSetChanged();
                        });

                        ActivityLog log = new ActivityLog();
                        log.setUserId(userId);
                        log.setAction("DECRYPT");
                    ...
                    final int index = i;
                    runOnUiThread(() -> {
                        displayItems.set(index, fileName + " - Decrypting...");
                        filesAdapter.notifyDataSetChanged();
                        statusLabel.setText("Decrypting " + fileName);
                    });
                    ...
                    try {
                        EncryptDecrypt.decryptFile(
                                ...
                                progress -> runOnUiThread(() -> {
                                    ...
                                    displayItems.set(index, fileName + " - Decrypting... " + percentage + "%");
                                    filesAdapter.notifyDataSetChanged();
                    ...
                    long decryptedFileSize = outputFile.length();

                    try {
                        fileDAO.updateForDecryption(
                                userId,
                                encryptedHash,
                                outputFile.getName(),
                                decryptedFileSize
                    ...
                    log.setFileName(fileName);
                    log.setFileSize(decryptedFileSize);
                    logDAO.logActivity(log);
